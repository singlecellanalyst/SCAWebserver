...
# scRNA-Seq
# Version: V1.0.0
# Creator: Lu Pan, Karolinska Institutet, eudoraleer@gmail.com
# Last Editded Date: 2023-02-19
########################################################################################################
  ...
  # Imputation package from ALRA: https://www.nature.com/articles/s41467-021-27729-z
  ...
  for(j in 1:nrow(pheno_data)){
    ...
    somePDFPath <- paste(cdir,"1SCA_PLOT_scRNASEQ_PREFILTERING_RNA_INFO_VIOLIN_",pheno_data[j,"SID"],"_",project_name,".pdf", sep = "")
    ...
    somePDFPath <- paste(cdir,"2SCA_PLOT_scRNASEQ_PREFILTERING_RNA_INFO_SCATTERED_",pheno_data[j,"SID"],"_",project_name,".pdf", sep = "")
    ...
    if(nrow(data_current[[j]]) > 2000){
      ...
      somePDFPath = paste(cdir,"3SCA_PLOT_scRNASEQ_CELL_CYCLE_PHASES_PCA_BEFORE_CC_REGRESSION_",annot_names[j],"_",project_name,".pdf", sep = "")
      ...
      somePDFPath = paste(cdir,"4SCA_PLOT_scRNASEQ_CELLCYCLE_UMAP_BEFORE_REGRESSION_",annot_names[j],"_",project_name,".pdf", sep = "")
      pdf(file=somePDFPath, width=10, height=7,pointsize=12)
      print(p1 / p2 + plot_annotation(title = paste("Before Cell Cycle Regression - ",annot_names[j], sep = ""), theme = theme(plot.title = element_text(size = 20, face = "bold", hjust = 0.5))))
      dev.off()
    ...
    somePDFPath = paste(cdir,"5SCA_PLOT_scRNASEQ_CELL_CYCLE_PHASES_UMAP_BEFORE_CC_REGRESSION_",annot_names[j],"_",project_name,".pdf", sep = "")
    ...
    if(ncol(data_current[[j]]) > 200){
      ...
      if(cc_regression != 0 & (nrow(data_current[[j]][['RNA']]) > 2000)){
        ...
        somePDFPath = paste(cdir,"6SCA_PLOT_scRNASEQ_CELL_CYCLE_PHASES_PCA_POST_CC_",gsub("\\s+","_",toupper(cc_type)),"_REGRESSION_", annot_names[j], "_", project_name,".pdf", sep = "")
        pdf(file=somePDFPath, width=10, height=7,pointsize=12)
        ...
        somePDFPath = paste(cdir,"7SCA_PLOT_scRNASEQ_CELLCYCLE_UMAP_POST_CC_",gsub("\\s+","_",toupper(cc_type)),"_REGRESSION_",annot_names[j],"_",project_name,".pdf", sep = "")
        pdf(file=somePDFPath, width=10, height=7,pointsize=12)
        print(p1 / p2 + plot_annotation(title = paste("Post Cell Cycle Regression: ",annot_names[j], sep = ""), theme = theme(plot.title = element_text(size = 20, face = "bold", hjust = 0.5))))
        dev.off()

        somePDFPath = paste(cdir,"8SCA_PLOT_scRNASEQ_CELL_CYCLE_PHASES_UMAP_POST_CC_",gsub("\\s+","_",toupper(cc_type)),"_REGRESSION_",annot_names[j],"_",project_name,".pdf", sep = "")
        pdf(file=somePDFPath, width=10, height=ceiling(length(metrics))/3*5,pointsize=12)
        print(FeaturePlot(data_current[[j]],
                          reduction = "umap",
                          features = metrics,
        ...
        somePDFPath = paste(cdir,"8SCA_PLOT_scRNASEQ_NO_CELL_CYCLE_REGRESSION_UMAP_POST_CC_NO_REGRESSION_",annot_names[j],"_",project_name,".pdf", sep = "")
        pdf(file=somePDFPath, width=10, height=ceiling(length(metrics))/3*5,pointsize=12)
        print(FeaturePlot(data_current[[j]],
                          reduction = "umap",
                          features = metrics,
      ...
      somePDFPath = paste(cdir,"9SCA_PLOT_scRNASEQ_POSTFILTERING_RNA_INFO_VIOLIN_",annot_names[j],"_",project_name,".pdf", sep = "")
      pdf(file=somePDFPath, width=14, height=8,pointsize=12)
      print(p+plot_annotation(title = annot_names[j], theme = theme(plot.title = element_text(size = 20, face = "bold", hjust = 0.5))))
      dev.off()
      ...
      somePDFPath = paste(cdir,"10SCA_PLOT_scRNASEQ_POSTFILTERING_RNA_INFO_SCATTERED_",annot_names[j],"_",project_name,".pdf", sep = "")
      pdf(file=somePDFPath, width=10, height=5,pointsize=12)
      ...
      somePDFPath = paste(cdir,"11SCA_PLOT_scRNASEQ_POSTNORMALIZED_VARIABLE_FEATURE_",annot_names[j],"_",project_name,".pdf", sep = "")
      pdf(file=somePDFPath, width=10, height=7,pointsize=12)
      ...
      somePDFPath = paste(cdir,"12SCA_PLOT_scRNASEQ_TOP_FEATURES_ASSOC_PC12_",annot_names[j],"_",project_name,".pdf", sep = "")
      ...
      somePDFPath = paste(cdir,"15SCA_PLOT_scRNASEQ_UMAP_AUTOCLUSTER_",annot_names[j],"_",project_name,".pdf", sep = "")
      ...
      somePDFPath = paste(cdir,"16SCA_PLOT_scRNASEQ_TSNE_AUTOCLUSTER_",annot_names[j],"_",project_name,".pdf", sep = "")
      pdf(file=somePDFPath, width=9, height=7,pointsize=12)
      ...
      write.table(current_data_markers, paste(cdir, "17SCA_TABLE_scRNASEQ_DEGS_NO_FILTER_",annot_names[j],"_",project_name,".txt", sep = ""), quote = F, row.names = F, sep = "\t")
      ...
      somePDFPath = paste(cdir,"18SCA_PLOT_scRNASEQ_HEATMAP_MEDIAN_EXPR_CLUSTER_TOP_", n, "_GENES_", annot_names[j], "_",project_name, ".pdf", sep = "")
      ...
      somePDFPath = paste(cdir,"19SCA_PLOT_scRNASEQ_AVE_LOGFC_TOP_GENES_TSNE_",annot_names[j],"_",project_name,".pdf", sep = "")
      pdf(file=somePDFPath, width=16, height=7,pointsize=12)
      grid.arrange(p1, p2, ncol = 2)
      dev.off()

      somePDFPath = paste(cdir,"20SCA_PLOT_scRNASEQ_AVE_LOGFC_TOP_GENES_UMAP_",annot_names[j],"_",project_name,".pdf", sep = "")
      pdf(file=somePDFPath, width=16, height=7,pointsize=12)
      ...
      somePDFPath = paste(cdir,"21SCA_PLOT_scRNASEQ_RIDGE_TOP_FC_SIG_GENES_IN_CLUSTERS_",annot_names[j],"_",project_name,".pdf", sep = "")
      ...
      somePDFPath = paste(cdir,"22SCA_PLOT_scRNASEQ_UMAP_DENSITY_TOP_1_MARKER_IN_CLUSTERS_",annot_names[j],"_",project_name,".pdf", sep = "")
...
if(run_unbias_vis == "YES"){
      ...
      somePDFPath = paste(cdir,"22SCA_PLOT_scRNASEQ_SCUBI_UNBIASED_VIS_UMAP_DENSITY_TOP_1_MARKER_IN_CLUSTERS_",annot_names[j],"_",project_name,".pdf", sep = "")
      pdf(file=somePDFPath, width=20, height=ceiling(length(top1$gene)/4)*5,pointsize=12)
      ...
      somePDFPath = paste(cdir,"23SCA_PLOT_scRNASEQ_VIOLIN_TOP_",n,"_MARKERS_IN_CLUSTERS_",annot_names[j],"_",project_name,".pdf", sep = "")
      ...
      somePDFPath = paste(cdir,"24SCA_PLOT_scRNASEQ_HEATMAP_TOP_",n,"_MARKERS_IN_CLUSTERS_",annot_names[j],"_",project_name,".pdf", sep = "")
      ...
      write.table(plotx, paste(cdir, "25SCA_TABLE_scRNASEQ_UMAP_TSNE_PCA_COORDINATES_CLUSTERS_",annot_names[j],"_",project_name,".txt", sep = ""), quote = F, row.names = F, sep = "\t")

      cct_colors <- gen_colors(n = length(unique(plotx$CELL_TYPE)))
      names(cct_colors) <- sort(unique(plotx$CELL_TYPE))

      somePDFPath = paste(cdir,"26SCA_PLOT_scRNASEQ_UMAP_AUTO_CELL_TYPE_IDENTIFICATION_",annot_names[j],"_",project_name,".pdf", sep = "")
      ...
      somePDFPath = paste(cdir,"27SCA_PLOT_scRNASEQ_TSNE_AUTO_CELL_TYPE_IDENTIFICATION_",annot_names[j],"_",project_name,".pdf", sep = "")
      pdf(file=somePDFPath, width=9, height=6,pointsize=12)
      ...
      n <- 15
      somePDFPath = paste(cdir,"28SCA_PLOT_scRNASEQ_TOP_",n,"_PATHWAYS_IN_EACH_CLUSTER_TOPFC200GENES_CLUSTER_",annot_names[j],"_",project_name,".pdf", sep = "")
      ...
      write.table(summary_pathways, paste(cdir, "29SCA_TABLE_scRNASEQ_TOP_ABS0.25_AVG_LOG2FC_ENRICHMENT_TERMS_",annot_names[j], "_",project_name,".txt",sep = ""), quote = F, row.names = F, sep = "\t")

      somePDFPath = paste(cdir,"30SCA_PLOT_scRNASEQ_DOT_PLOT_TOP_ABS0.25_AVG_LOG2FC_PATHWAYS_IN_EACH_CLUSTER_",annot_names[j],"_",project_name,".pdf", sep = "")
      pdf(file=somePDFPath, width=12, height=6,pointsize=12)
      for(i in 1:length(pathway_EA_result)){
        current <- pathway_EA_result[[i]]
        if(!is.null(current)){
      ...
      somePDFPath = paste(cdir,"31SCA_PLOT_scRNASEQ_TOP_ABS0.25_AVG_LOG2FC_GENES_CONCEPT_NETWORK_",annot_names[j],"_",project_name,".pdf", sep = "")
      ...
      somePDFPath = paste(cdir,"32SCA_PLOT_scRNASEQ_TOP_ABS0.25_AVG_LOG2FC_NETWORK_ENRICHMENT_TERMS_",annot_names[j],"_",project_name,".pdf", sep = "")
      ...
      somePDFPath = paste(cdir,"33SCA_PLOT_scRNASEQ_HEATMAP_TOP_ABS0.25_AVG_LOG2FC_ENRICHED_NETWORK_TERMS_BY_CLUSTER_",annot_names[j],"_",project_name,".pdf", sep = "")
      ...
      somePDFPath = paste(cdir,"34SCA_PLOT_scRNASEQ_TOP_ABS0.25_AVG_LOG2FC_ENRICHMENT_PROPORTIONS_GENES_CLUSTER_",annot_names[j],"_",project_name,".pdf", sep = "")
      ...
      somePDFPath = paste(cdir,"35SCA_PLOT_scRNASEQ_TOP_ABS0.25_AVG_LOG2FC_ENRICHMENT_PROPORTIONS_GENES_CLUSTER_",annot_names[j],"_",project_name,".pdf", sep = "")
      ...
      somePDFPath = paste(cdir,"36SCA_PLOT_scRNASEQ_TOP_ABS0.25_AVG_LOG2FC_KEGG_ENRICHMENT_MAP_PROPORTION_CLUSTER_",annot_names[j],"_",project_name,".pdf", sep = "")
      pdf(file=somePDFPath, width=10, height=6,pointsize=12)
      for(i in 1:length(pathway_EA_result)){
        if(!is.null(pathway_EA_result[[i]])){
      ...
      if(!all(if_plot == FALSE)){
        ...
        somePDFPath = paste(cdir,"37SCA_PLOT_scRNASEQ_TOP_ABS0.25_AVG_LOG2FC_GSEA_GENE_ENRICHMENT_SCORE_",annot_names[j],"_",project_name,".pdf", sep = "")
        pdf(file=somePDFPath, width=10, height=7,pointsize=12)
        for(i in 1:length(GSEA_result)){
...
ggtitle(paste(annot_names[j], "\nGSEA Plot: Cluster ", names(pathway_EA_result)[i],sep = "")) +
      ...
      if(length(unique(data_current[[j]]$CELL_TYPE)) > 1){
        ...
        somePDFPath = paste(cdir,"39SCA_PLOT_scRNASEQ_AVE_LOGFC_TOP_GENES_TSNE_CELL_TYPES_",annot_names[j],"_",project_name,".pdf", sep = "")
        pdf(file=somePDFPath, width=24, height=ceiling(length(unique(plotx$CELL_TYPE))/3)*5,pointsize=12)
        grid.arrange(p1, p2, ncol = 2)
        dev.off()

        somePDFPath = paste(cdir,"40SCA_PLOT_scRNASEQ_AVE_LOGFC_TOP_GENES_UMAP_CELL_TYPES_",annot_names[j],"_",project_name,".pdf", sep = "")
        pdf(file=somePDFPath, width=24, height=ceiling(length(unique(plotx$CELL_TYPE))/3)*5,pointsize=12)
        grid.arrange(p1, p3, ncol = 2)
        dev.off()
        ...
        somePDFPath = paste(cdir,"41SCA_PLOT_scRNASEQ_RIDGE_TOP_FC_SIG_GENES_CELL_TYPES_",annot_names[j],"_",project_name,".pdf", sep = "")
  ...
  if(nrow(pheno_data) == 1){
    ...
    write.table(data_dim, paste(cdir, "42SCA_TABLE_scRNASEQ_DIM_PARAMETERS_BEFORE_INTEGRATION_", project_name,".txt", sep = ""), quote = F, row.names = T, sep = "\t")
    ...
    write.table(current, paste(cdir, "43SCA_TABLE_scRNASEQ_DIM_PARAMETERS_AFTER_", integration_method, "_INTEGRATION_", project_name,".txt", sep = ""), quote = F, row.names = T, sep = "\t")

    data_dim <- rbind(data_dim, cbind(data.frame(gen10x_plotx(data), DATA_TYPE = integration_name, SAMPLE_ID = data$orig.ident)))

    somePDFPath = paste(cdir,"44SCA_PLOT_scRNASEQ_UMAP_BEFORE_AFTER_",integration_method, "_INTEGRATION_", project_name,".pdf", sep = "")
    ...
    somePDFPath = paste(cdir,"45SCA_PLOT_scRNASEQ_TSNE_BEFORE_AFTER_",integration_method, "_INTEGRATION_", project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=20, height=10,pointsize=12)
    print(beforeafter_dimplot(data_dim[data_dim$DATA_TYPE == "BEFORE_INTEGRATION",],
                              data_dim[data_dim$DATA_TYPE == integration_name,],
    ...
    if(toupper(integration_method) == "SEURAT"){
      ...
      somePDFPath = paste(cdir,"46SCA_PLOT_scRNASEQ_PCA_BEFORE_AFTER_",integration_method, "_INTEGRATION_", project_name,".pdf", sep = "")
    ...
    somePDFPath = paste(cdir,"47SCA_PLOT_scRNASEQ_BY_GROUP_UMAP_BEFORE_AFTER_",integration_method, "_INTEGRATION_", project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=30, height=15,pointsize=12)
    print(beforeafter_dimplot(data_dim[data_dim$DATA_TYPE == "BEFORE_INTEGRATION",],
                              data_dim[data_dim$DATA_TYPE == integration_name,],
    ...
    somePDFPath = paste(cdir,"48SCA_PLOT_scRNASEQ_BY_GROUP_TSNE_BEFORE_AFTER_",integration_method, "_INTEGRATION_", project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=30, height=15,pointsize=12)
    print(beforeafter_dimplot(data_dim[data_dim$DATA_TYPE == "BEFORE_INTEGRATION",],
                                                 data_dim[data_dim$DATA_TYPE == integration_name,],
    ...
    somePDFPath = paste(cdir,"49SCA_PLOT_scRNASEQ_BY_GROUP_PCA_BEFORE_AFTER_",integration_method, "_INTEGRATION_", project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=30, height=15,pointsize=12)
    print(p1+p2+
            plot_annotation(title = project_name, theme = theme(plot.title = element_text(size = 35, face = "bold", hjust = 0.5))))
    dev.off()
    ...
    somePDFPath = paste(cdir,"50SCA_PLOT_scRNASEQ_BY_BATCH_UMAP_BEFORE_AFTER_",integration_method, "_INTEGRATION_", project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=30, height=15,pointsize=12)
    print(beforeafter_dimplot(data_dim[data_dim$DATA_TYPE == "BEFORE_INTEGRATION",],
                              data_dim[data_dim$DATA_TYPE == integration_name,],
    ...
    somePDFPath = paste(cdir,"51SCA_PLOT_scRNASEQ_BY_BATCH_TSNE_BEFORE_AFTER_",integration_method, "_INTEGRATION_", project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=30, height=15,pointsize=12)
    print(beforeafter_dimplot(data_dim[data_dim$DATA_TYPE == "BEFORE_INTEGRATION",],
                                                 data_dim[data_dim$DATA_TYPE == integration_name,],
    ...
    somePDFPath = paste(cdir,"52SCA_PLOT_scRNASEQ_BY_BATCH_PCA_BEFORE_AFTER_",integration_method, "_INTEGRATION_", project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=30, height=15,pointsize=12)
    print(p1+p2+
            plot_annotation(title = project_name, theme = theme(plot.title = element_text(size = 35, face = "bold", hjust = 0.5))))
    dev.off()
    ...
    somePDFPath = paste(cdir,"53SCA_PLOT_scRNASEQ_INTEGRATED_DATA_TOP_FEATURES_ASSOC_PC12_",integration_name,"_",project_name,".pdf", sep = "")
    ...
    ############## Integration Plots #################################################################
    DefaultAssay(data) <- "integrated"
    somePDFPath = paste(cdir,"54SCA_PLOT_scRNASEQ_HEATMAP_PC1_TO_15_TOP_FEATURES_INTEGRATED_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=8, height=16,pointsize=12)
    ...
    somePDFPath = paste(cdir,"55SCA_PLOT_scRNASEQ_UMAP_AUTOCLUSTER_",integration_name,"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=12, height=10,pointsize=12)
    print(p1)
    dev.off()

    somePDFPath = paste(cdir,"56SCA_PLOT_scRNASEQ_UMAP_AUTOCLUSTER_",integration_name,"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=12, height=10,pointsize=12)
    print(p2)
    dev.off()
    ...
    write.table(current_data_markers, paste(cdir, "57SCA_TABLE_scRNASEQ_AUTO_CLUSTER_TOP_MARKERS_",integration_name,"_",project_name,".txt", sep = ""), quote = F, row.names = F, sep = "\t")

    somePDFPath = paste(cdir,"58SCA_PLOT_scRNASEQ_UMAP_DENSITY_TOP_1_MARKER_IN_CLUSTERS_",integration_name,"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=20, height=ceiling(length(top1$gene)/4)*5,pointsize=12)
    print(current_out$featureplot+plot_layout(ncol = 4) +
      plot_annotation(title = project_name, theme = theme(plot.title = element_text(size = 35, face = "bold", hjust = 0.5))))
    dev.off()

    somePDFPath = paste(cdir,"59SCA_PLOT_scRNASEQ_VIOLIN_TOP_",n,"_MARKERS_IN_CLUSTERS_",integration_name,"_",project_name,".pdf", sep = "")
    ...
    somePDFPath = paste(cdir,"60SCA_PLOT_scRNASEQ_HEATMAP_TOP_",n,"_MARKERS_IN_CLUSTERS_",integration_name,"_",project_name,".pdf", sep = "")
    ...
    write.table(data.frame(plotx), paste(cdir, "61SCA_TABLE_scRNASEQ_AUTO_ANNOTATION_CELL_TYPE_META_",integration_name,"_",project_name,".txt", sep = ""), quote = F, row.names = T, sep = "\t")

    ct_colors <- gen_colors(n = length(unique(plotx$INTEGRATED_CELL_TYPE)))
    names(ct_colors) <- sort(unique((plotx$INTEGRATED_CELL_TYPE)))

    somePDFPath = paste(cdir,"62SCA_PLOT_scRNASEQ_UMAP_AUTO_CELL_TYPE_IDENTIFICATION_",integration_name,"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=16, height=12,pointsize=12)
    ...
    somePDFPath = paste(cdir,"63SCA_PLOT_scRNASEQ_TSNE_AUTO_CELL_TYPE_IDENTIFICATION_",integration_name,"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=16, height=12,pointsize=12)
    print(plot_bygroup(plotx, x = "tSNE_1", y = "tSNE_2", group = "INTEGRATED_CELL_TYPE", plot_title = project_name,
                                          col = ct_colors, annot = TRUE, legend_position = "right", point_size = 1))
    dev.off()

    somePDFPath = paste(cdir,"64SCA_PLOT_scRNASEQ_UMAP_BY_GROUP_AUTO_CELL_TYPE_",integration_name,"_",project_name,".pdf", sep = "")
    ...
    somePDFPath = paste(cdir,"64SCA_PLOT_scRNASEQ_RECEPTOR_LIGAND_TOP_INTERACTIONS_INTEGRATED_DATA_CELL_TYPES_", integration_name, project_name, ".pdf", sep = "")
    ...
    write.table(data_interactions, paste(cdir, "64SCA_PLOT_scRNASEQ_RECEPTOR_LIGAND_P005_INTERACTIONS_INTEGRATED_DATA_CELL_TYPES_",integration_name,"_",project_name,".txt", sep = ""), quote = F, row.names = T, sep = "\t")
    ...
    somePDFPath = paste(cdir,"65SCA_PLOT_scRNASEQ_HEATMAP_MEDIAN_EXPR_CLUSTER_TOP_FOLDCHANGE_SIG_GENES_", integration_name, project_name, ".pdf", sep = "")
    ...
    somePDFPath = paste(cdir,"66SCA_PLOT_scRNASEQ_AVE_LOGFC_TOP_GENES_TSNE_",integration_name,"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=20, height=9,pointsize=12)
    grid.arrange(p1, p2, ncol = 2)
    dev.off()

    somePDFPath = paste(cdir,"67SCA_PLOT_scRNASEQ_AVE_LOGFC_TOP_GENES_UMAP_",integration_name,"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=20, height=9,pointsize=12)
    grid.arrange(p1, p3, ncol = 2)
    dev.off()
    ...
    Idents(data) <- "seurat_clusters"
    somePDFPath = paste(cdir,"68SCA_PLOT_scRNASEQ_RIDGE_TOP_FC_SIG_GENES_IN_CLUSTERS_",integration_name,"_",project_name,".pdf", sep = "")
    ...
    somePDFPath = paste(cdir,"69SCA_PLOT_scRNASEQ_NODE_SUMMARY_SAMPLE_PROPORTION_",integration_name, "_",project_name,".pdf", sep = "")
    ...
    somePDFPath = paste(cdir,"70SCA_PLOT_scRNASEQ_CELL_TYPE_SUMMARY_SAMPLE_PROPORTION_",integration_name, "_",project_name,".pdf", sep = "")
    ...
    top_markers_cell_type <- current$top_markers
    plot_median_cell_type[is.na(plot_median_cell_type)] <- 0
    somePDFPath = paste(cdir,"71SCA_PLOT_scRNASEQ_HEATMAP_MEDIAN_EXPR_CELL_TYPE_TOP_FOLDCHANGE_SIG_GENES_", integration_name, project_name, ".pdf", sep = "")
    pdf(file=somePDFPath, width=8, height=10,pointsize=12)
    print(complex_heatmap(plot_median_cell_type, col = color_conditions$BlueYellowRed))
    dev.off()
    ...
    somePDFPath = paste(cdir,"72SCA_PLOT_scRNASEQ_AVE_LOGFC_TOP_GENES_TSNE_CELL_TYPES_",integration_name,"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=24, height=ceiling(length(unique(plotx$CELL_TYPE))/3)*5,pointsize=12)
    grid.arrange(p1, p2, ncol = 2)
    dev.off()

    somePDFPath = paste(cdir,"73SCA_PLOT_scRNASEQ_AVE_LOGFC_TOP_GENES_UMAP_CELL_TYPES_",integration_name,"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=24, height=ceiling(length(unique(plotx$CELL_TYPE))/3)*5,pointsize=12)
    grid.arrange(p1, p3, ncol = 2)
    dev.off()
    ...
    somePDFPath = paste(cdir,"74SCA_PLOT_scRNASEQ_RIDGE_TOP_FC_SIG_GENES_CELL_TYPES_",integration_name,"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=16, height=length(unique(unlist(top_1_markers$gene)))/2*6,pointsize=12)
    ...
    somePDFPath = paste(cdir,"75SCA_PLOT_scRNASEQ_CELL_TYPE_SAMPLE_PROPORTION_",integration_name, "_",project_name,".pdf", sep = "")
    pdf(somePDFPath, width = 25, height = 20, pointsize = 10)
    ...
    if(length(unique(data$GROUP)) > 1){
      ...
      current_data_markers <- current_out$current_data_markers
      current_data_markers <- current_data_markers[order(current_data_markers$avg_log2FC, decreasing = T),]
      write.table(current_data_markers, paste(cdir, "76SCA_TABLE_scRNASEQ_BY_CLUSTERS_TOP_MARKERS_DE_GROUPS_EACH_CLUSTER_",integration_name,"_",project_name,".txt", sep = ""), quote = F, row.names = F, sep = "\t")
      ...
      somePNGPath <- paste(cdir,"76SCA_PLOT_scRNASEQ_CLUSTERS_BY_GROUP_TOP1_FEATURE_DIFFERENCE_COLOR_EXPR_",integration_name,"_",project_name,".png", sep = "")
      ...
      somePDFPath = paste(cdir,"77SCA_PLOT_scRNASEQ_VIOLIN_TOP_",n,"_MARKERS_DE_GROUPS_EACH_CLUSTER_",integration_name,"_",project_name,".pdf", sep = "")
      pdf(file=somePDFPath, width=16, height=10,pointsize=12)
      for(k in 1:length(current_clusters)){
        current <- topn[which(topn$cluster == current_clusters[k]),]
        current <- current[order(current$p_val_adj, decreasing = F),]
      ...
      somePDFPath = paste(cdir,"78SCA_PLOT_scRNASEQ_UMAP_DENSITY_TOP_1_MARKER_BY_DE_GROUP_EACH_CLUSTER_",integration_name,"_",project_name,".pdf", sep = "")
      pdf(file=somePDFPath, width=10, height=5,pointsize=12)
  ...
  for(j in 1:length(data_current)){
    ...
    p <- NULL
    somePDFPath = paste(cdir,"79SCA_PLOT_scRNASEQ_UMAP_TRAJECTORY_CLUSTERS_",annot_names[j],"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=12, height=10,pointsize=12)
    ...
    htmlwidgets::saveWidget(p, paste(cdir, "80SCA_PLOT_scRNASEQ_UMAP_TRAJECTORY_CLUSTERS_",annot_names[j],"_",project_name,".html", sep = ""))
    ...
    somePDFPath = paste(cdir,"81SCA_PLOT_scRNASEQ_UMAP_TRAJECTORY_CELL_TYPE_",annot_names[j],"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=12, height=10,pointsize=12)
    print(p)
    dev.off()
    ...
    p <- ggplotly(p+theme(legend.position = "right", plot.margin=unit(c(1,5,1,1),"cm"))+
                                             ...
                                             ggtitle(paste("<b>",names(data_current)[j],"</b>",sep = "")))
    htmlwidgets::saveWidget(p, paste(cdir, "82SCA_PLOT_scRNASEQ_UMAP_TRAJECTORY_CELL_TYPE_",annot_names[j],"_",project_name,".html", sep = ""))

    current_clusters <- sort(as.numeric(as.character(unique(mono3_current[[j]]@clusters$UMAP$clusters))), decreasing = F)

    somePDFPath = paste(cdir,"83SCA_PLOT_scRNASEQ_UMAP_PSEUDOTIME_EACH_CLUSTER_AS_ROOT_", annot_names[j],"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=12, height=10,pointsize=12)
...
chosen_para_names <- c("CLUSTERS", "CELL_TYPE")
    ...
    somePDFPath = paste(cdir,"84SCA_PLOT_scRNASEQ_PSEUDOTIME_RANKED_PCA_UMAP_TSNE_",annot_names[j],"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=8, height=6,pointsize=12)
...
somePDFPath = paste(cdir,"85SCA_PLOT_scRNASEQ_UMAP_PSEUDOTIME_EVERY_CELL_TYPE_AS_ROOT_", annot_names[j],"_",project_name,".pdf", sep = "")
pdf(file=somePDFPath, width=12, height=10,pointsize=12)
    ...
    htmlwidgets::saveWidget(p, paste(cdir, "86SCA_PLOT_scRNASEQ_UMAP_INTERACTIVE_CELL_TYPES_",annot_names[j],"_",project_name,".html", sep = ""))
    ...
    htmlwidgets::saveWidget(p, paste(cdir, "87SCA_PLOT_scRNASEQ_TSNE_INTERACTIVE_CELL_TYPES_",annot_names[j],"_",project_name,".html", sep = ""))
    ...
    htmlwidgets::saveWidget(p, paste(cdir, "88SCA_PLOT_scRNASEQ_UMAP_INTERACTIVE_CLUSTERS_",annot_names[j],"_",project_name,".html", sep = ""))
    ...
    p <- plot_3d(data_current[[j]]@reductions$tsne@cell.embeddings[,"tSNE_1"],
                                          ...
                                          current_text= paste("Cell: ",row.names(data_current[[j]]@meta.data),"\nCluster: ",
                                                              data_current[[j]]$seurat_clusters, sep = ""))
    htmlwidgets::saveWidget(p, paste(cdir, "89SCA_PLOT_scRNASEQ_TSNE_INTERACTIVE_CLUSTERS_",annot_names[j],"_",project_name,".html", sep = ""))
  ...
  if(length(data_current) > 1){
    ...
    somePDFPath = paste(cdir,"90SCA_PLOT_scRNASEQ_INTEGRATED_DATA_PSEUDOTIME_RANKED_PCA_UMAP_TSNE_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=8, height=6,pointsize=12)
    for(m in 1:length(chosen_para)){
      for(n in 1:length(pseudo_para)){
        p <- NULL
    ...
    somePDFPath = paste(cdir,"91SCA_PLOT_scRNASEQ_INTEGRATED_DATA_UMAP_TRAJECTORY_CLUSTERS_",integration_name,"_",project_name,".pdf", sep = "")
    ...
    htmlwidgets::saveWidget(p, paste(cdir, "92SCA_PLOT_scRNASEQ_INTEGRATED_DATA_UMAP_TRAJECTORY_CLUSTERS_",integration_name,"_",project_name,".html", sep = ""))
    ...
    somePDFPath = paste(cdir,"93SCA_PLOT_scRNASEQ_INTEGRATED_DATA_UMAP_TRAJECTORY_CELL_TYPE_",integration_name,"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=9*length(unique(mono3data@colData$GROUP)), height=8,pointsize=12)
    print(p)
    dev.off()
    ...
    p <- ggplotly(p+theme(legend.position = "right", plot.margin=unit(c(1,5,1,1),"cm"))+
                                        ggtitle(paste("<b>",project_name,"</b>",sep = "")))
    htmlwidgets::saveWidget(p, paste(cdir, "94SCA_PLOT_scRNASEQ_INTEGRATED_DATA_UMAP_TRAJECTORY_CELL_TYPE_",integration_name,"_",project_name,".html", sep = ""))

    current_clusters <- sort(as.numeric(as.character(unique(mono3data@clusters$UMAP$clusters))), decreasing = F)

    somePDFPath = paste(cdir,"95SCA_PLOT_scRNASEQ_INTEGRATED_DATA_UMAP_PSEUDOTIME_EVERY_CLUSTER_AS_ROOT_", integration_name,"_",project_name,".pdf", sep = "")
    pdf(file=somePDFPath, width=9*length(unique(mono3data@colData$GROUP)), height=8,pointsize=12)
    for(i in 1:length(current_clusters)){
    ...
    htmlwidgets::saveWidget(p, paste(cdir, "96SCA_PLOT_scRNASEQ_UMAP_INTERACTIVE_CELL_TYPES_",integration_name,"_",project_name,".html", sep = ""))
    ...
    p <- plot_3d(data@reductions$tsne@cell.embeddings[,"tSNE_1"],
                                     ...
                                     current_text= paste("Cluster ",data@meta.data[,integration_cluster],"\n",
                                                         "Cell: ",row.names(data@meta.data),"\n",
                                                         data@meta.data[,"CELL_TYPE"], sep = ""))
    htmlwidgets::saveWidget(p, paste(cdir, "97SCA_PLOT_scRNASEQ_TSNE_INTERACTIVE_CELL_TYPES_",integration_name,"_",project_name,".html", sep = ""))
    ...
    htmlwidgets::saveWidget(p, paste(cdir, "98SCA_PLOT_scRNASEQ_UMAP_INTERACTIVE_CLUSTERS_",integration_name,"_",project_name,".html", sep = ""))
    ...
    p <- plot_3d(data@reductions$tsne@cell.embeddings[,"tSNE_1"],
                                     ...
                                     current_text= paste("Cell: ",row.names(data@meta.data),"\nCluster: ",
                                                         data@meta.data[,integration_cluster], sep = ""))
    htmlwidgets::saveWidget(p, paste(cdir, "99SCA_PLOT_scRNASEQ_TSNE_INTERACTIVE_CLUSTERS_",integration_name,"_",project_name,".html", sep = ""))

  }

  saveRDS(data, paste(cdir, "100SCA_DATA_scRNASEQ_",project_name, ".RDS", sep = ""))
