...
Packages <- c("Seurat","ggplot2","patchwork","dplyr","ggridges",
              "ggthemes","gridExtra","reshape2", "ggrepel","scales","xlsx","ComplexHeatmap") #"SeuratData",

suppressMessages(lapply(Packages, library, character.only = TRUE))

check_zip <- function(cfile, targeted_dir, type = "DEFAULT"){
  
  # gz_files <- list.files(path = targeted_dir, recursive = T, pattern = "\\.gz|\\.zip|\\.tar|\\.gzip|\\.rar|\\.7z|\\.bz2|\\.tar\\.gz", full.names = T, ignore.case = T)
  
  if(length(cfile) == 0){
    print("No targeted zipped files found")
  }else{
    print("Zipped files found!")
    # lapply(gz_files, function(x){
      # ccdir <- paste(targeted_dir, gsub(".*\\/(RNA\\/.*)(\\.tar|\\.zip|\\.gzip).*","\\1",x), "/",sep = "")
      # ccdir <- gsub("(.*)\\.gz$","\\1",ccdir, ignore.case = T)
      dir.create(targeted_dir)
      print("targeted_dir:")
      print(targeted_dir)
      system(paste("tar -xvzf \"", cfile,"\" -C \"", targeted_dir,"\"",sep = ""))
      system(paste("rm \"", cfile, "\"",sep = ""))
    # })
    
    if(dir.exists(paste(targeted_dir, "__MACOSX", sep = ""))){
      system(paste("rm -rf \"", targeted_dir, "__MACOSX\"", sep = ""))
    }
