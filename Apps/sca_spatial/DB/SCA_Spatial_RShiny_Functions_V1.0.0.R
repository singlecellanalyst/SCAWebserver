...
pheno_ini <- function(phenodata_dir, pipeline, extra_para = NULL, isDir = TRUE){
  ...
  if(toupper(pipeline) == "SPATIAL"){
    ...
    if(!is.null(extra_para)){
      colnames(pheno_data) <- c("FILE","SAMPLE_ID","INDIVIDUAL_ID","BATCH","GROUP",extra_para)
    }else{
      colnames(pheno_data) <- c("FILE","SAMPLE_ID","INDIVIDUAL_ID","BATCH","GROUP")
    }
...
own_facet_scatter <- function(plotx, feature1, feature2, isfacet = T, title, col=color_conditions$bright, color_by, group_by = NULL, xlabel = feature1, ylabel = feature2, strip_size = 15, facet_col = 4){
  # set.seed(2022)
  ...
  if(isfacet == T & !is.null(group_by)){
    p <- p + facet_wrap(~plotx[,group_by], ncol = facet_col)
  }
  return(p)
}

adjust_theme <- function(p, xangle = 0,legend = "right", title_size = 20, xsize=20, hejust = 0, vejust = 0){
  p <- p+ theme_classic()+
    # ggtitle(title) +
