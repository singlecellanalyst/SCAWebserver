...
shinyServer(function(input, output, session) {
  ...
  output$downloadExample1 <- downloadHandler(
    filename = "SCA_CyTOF_Example_From_10X.zip",
  ...
  data <- reactive({
    ...
    EXPR_PARA <- data_submit[,grep("FILE|time|cell_length|dna|Event_length|Center|Offset|Width|Residual|^BC[0-9]+$|^[A-Z]+[0-9]+DI$|bead|L\\/D|Dead|Live", colnames(data_submit), ignore.case = T)]
    
    cofactor <- 5
    data <- asinh(data_submit[,grep("FILE|time|cell_length|dna|Event_length|Center|Offset|Width|Residual|^BC[0-9]+$|^[A-Z]+[0-9]+DI$|bead|L\\/D|Dead|Live", colnames(data_submit), ignore.case = T, invert = T)] / cofactor)
    ...
    showModal(modalDialog("Generating summary statistics..", footer=NULL))
    ...
    showModal(modalDialog("Dimension reduction and clustering..", footer=NULL))
    ...
    clusters <- sort(unique(results$p20data$Cluster))
  ...
  observeEvent(input$p1plot, {
    screenshot(id="p1plot", filename = paste("1SCA_CyTOF_SAMPLE_CELL_SUMMARY_",data()[['project_name']], sep = ""), scale = 2)
  })
  ...
  observeEvent(input$p2plot, {
    screenshot(id="p2plot", filename = paste("2SCA_CyTOF_MARKER_DENSITIES_",data()[['project_name']], sep = ""), scale = 2)
  })
  ...
  observeEvent(input$p3plot, {
    screenshot(id="p3plot", filename = paste("3SCA_CyTOF_MDS_SAMPLE_DISTANCE_",data()[['project_name']], sep = ""), scale = 2)
  })
  ...
  observeEvent(input$p4plot, {
    screenshot(id="p4plot", filename = paste("4SCA_CyTOF_PCA_SAMPLE_DISTANCE_",data()[['project_name']], sep = ""), scale = 2)
  })
  ...
  observeEvent(input$p5plot, {
    screenshot(id="p5plot", filename = paste("5SCA_CyTOF_HEATMAP_SAMPLE_DISTANCE_ARCSINH_MEDIAN_",data()[['project_name']], sep = ""), scale = 2)
  })
  ...
  observeEvent(input$p6plot, {
    screenshot(id="p6plot", filename = paste("6SCA_CyTOF_BOXPLOT_MARKERS_BY_GROUP_",data()[['project_name']], sep = ""), scale = 2)
  })
  
  output$p7plot <- renderPlot({
    showModal(modalDialog("Plotting figure 7..", footer=NULL))
    par(mar=c(3,4,1,6))
    print(plot(data()[['p7data']], horiz = TRUE))
    removeModal()
    return(p)
  }, height = 1000, width = 1000)
  
  observeEvent(input$p7plot, {
    screenshot(id="p7plot", filename = paste("7SCA_CyTOF_DENDROGRAM_SAMPLES_",data()[['project_name']], sep = ""), scale = 2)
  })
  ...
  observeEvent(input$p8plot, {
    screenshot(id="p8plot", filename = paste("8SCA_CyTOF_NRS_RANKING_ALL_MARKERS_",data()[['project_name']], sep = ""), scale = 2)
  })
...
observeEvent(input$p9plot, {
  screenshot(id="p9plot", filename = paste("9SCA_CyTOF_PAC_ELBOW_PLOT_CHOSEN_CLUSTER_NUMBER_",data()[['project_name']], sep = ""), scale = 2)
})
...
observeEvent(input$p10plot, {
  screenshot(id="p10plot", filename = paste("10SCA_CyTOF_HEATMAP_CLUSTERS_ARCSINH_MEDIAN_",data()[['project_name']], sep = ""), scale = 2)
})
...
observeEvent(input$p11plot, {
  screenshot(id="p11plot", filename = paste("11SCA_CyTOF_BOXPLOT_CLUSTERS_BY_GROUP_",data()[['project_name']], sep = ""), scale = 2)
})
...
output$p11bplot <- renderPlot({
  ...
  p <- p + facet_wrap(~ MARKER, scales = "free", ncol = 6) + stat_compare_means() + theme_classic() +
    scale_fill_manual(values = data()[['group_colors']])+
    scale_color_manual(values = data()[['sample_colors']])
...
observeEvent(input$p12plot, {
  screenshot(id="p12plot", filename = paste("12SCA_CyTOF_UMAP_BY_GROUP_",data()[['project_name']], sep = ""), scale = 2)
})
...
observeEvent(input$p13plot, {
  screenshot(id="p13plot", filename = paste("13SCA_CyTOF_UMAP_BY_SAMPLE_",data()[['project_name']], sep = ""), scale = 2)
})
...
observeEvent(input$p14plot, {
  screenshot(id="p14plot", filename = paste("14SCA_CyTOF_UMAP_BY_CLUSTER_",data()[['project_name']], sep = ""), scale = 2)
})
...
observeEvent(input$p15plot, {
  screenshot(id="p15plot", filename = paste("15SCA_CyTOF_PCA_BY_CLUSTER_",data()[['project_name']], sep = ""), scale = 2)
})
...
if(input$p16id != ""){
  ...
  plotx <- data()[['p16data']][which(data()[['p16data']]$GROUP == input$p16id),]
...
observeEvent(input$p17plot, {
  screenshot(id="p17plot", filename = paste("17SCA_CyTOF_PROPORTION_OF_SAMPLES_IN_EACH_CLUSTER_PIE_",data()[['project_name']], sep = ""), scale = 2)
})
...
observeEvent(input$p17bplot, {
  screenshot(id="p17bplot", filename = paste("17SCA_CyTOF_PROPORTION_OF_SAMPLES_IN_EACH_CLUSTER_BARPLOT_",data()[['project_name']], sep = ""), scale = 2)
})
...
observeEvent(input$p18plot, {
  screenshot(id="p18plot", filename = paste("18SCA_CyTOF_PROPORTION_OF_CLUSTERS_IN_EACH_SAMPLE_",data()[['project_name']], sep = ""), scale = 2)
})
...
observeEvent(input$p19plot, {
  screenshot(id="p19plot", filename = paste("19SCA_CyTOF_tSNE_SOM_NODES_",data()[['project_name']], sep = ""), scale = 2)
})

output$p20table <- renderDT(data()[['p20data']],
                            filter = "top",
                            style="bootstrap",
                            rownames = F,
...
observeEvent(input$p21plot, {
  screenshot(id="p21plot", filename = paste("21SCA_CyTOF_UMAP_SOM_NODES_",data()[['project_name']], sep = ""), scale = 2)
})
...
observeEvent(input$p22plot, {
  screenshot(id="p22plot", filename = paste("22SCA_CyTOF_PCA_SOM_NODES_",data()[['project_name']], sep = ""), scale = 2)
})
