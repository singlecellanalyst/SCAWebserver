...
########################################################################################################
  ...
  for(i in 1:nrow(pheno_data)){
    ...
    somePNGPath <- paste(cdir,"1SCA_PLOT_scATAC_NORM_TSS_ENRICHMENT_SCORES_",pheno_data[i,"SID"],"_",project_name,".png", sep = "")
    ...
    somePNGPath <- paste(cdir,"2SCA_PLOT_scATAC_FRAGMENT_LENGTH_PERIODICITY_",pheno_data[i,"SID"],"_",project_name,".png", sep = "")
    png(somePNGPath, width = 2000, height = 1000, units = "px", res = 300)
    ...
    somePNGPath <- paste(cdir,"3SCA_PLOT_scATAC_QUALITY_CONTROL_PEAKS_LOGSCALE_",pheno_data[i,"SID"],"_",project_name,".png", sep = "")
    ...
    somePNGPath <- paste(cdir,"4SCA_PLOT_scATAC_CORRELATION_SEQUENCING_DEPTH_LSI_COMPONENTS_",pheno_data[i,"SID"],"_",project_name,".png", sep = "")
    png(somePNGPath, width = 2000, height = 1000, units = "px", res = 300)
    ...
    if(nrow(da_peaks) > 0){
      ...
      write.csv(results[['p6data']][[i]], paste(cdir, "6SCA_TABLE_scATAC_TOP_PEAKS_IN_CLUSTERS_",pheno_data[i,"SID"],".csv",sep = ""), row.names = F, quote = F)
      ...
      write.csv(results[['p7data']][[i]], paste(cdir, "7SCA_TABLE_scATAC_TOP_",n,"_PEAKS_IN_CLUSTERS_EASY_TABLE_",pheno_data[i,"SID"],".csv",sep = ""), row.names = F, quote = F)
      ...
      write.csv(results[['p8data']][[i]], paste(cdir, "8SCA_TABLE_scATAC_CLOSEST_GENE_NEAR_TOP_PEAKS_IN_CLUSTERS_",pheno_data[i,"SID"],".csv",sep = ""), row.names = F, quote = F)
    }
    ...
    write.csv(results[['p9data']][[i]], paste(cdir, "9SCA_TABLE_scATAC_TOP_GENE_ACTIVITY_IN_CLUSTERS_",pheno_data[i,"SID"],".csv",sep = ""), row.names = F, quote = F)
    ...
    for(k in 1:length(clusters)){
      if(length(which(unlist(top1$cluster) == clusters[k])) > 0){
        ...
        somePNGPath <- paste(cdir,"10SCA_PLOT_scATAC_INTEGRATED_SAMPLES_Tn5_INSERTION_COVERAGE_TOP1_PEAK_TOP1_GENE_ACTIVITY_IN_CLUSTER_",clusters[k],"_",pheno_data[i,"SID"],"_",project_name,".png", sep = "")
    ...
    write.csv(results[['p11data']][[i]], paste(cdir, "11SCA_TABLE_scATAC_TOP_",n,"_GENES_IN_CLUSTERS_EASY_TABLE_",pheno_data[i,"SID"],".csv",sep = ""), row.names = F, quote = F)
    ...
    somePNGPath <- paste(cdir,"12SCA_PLOT_scATAC_",toupper(current_assay),"_TOP_",used_type,"_GENES_FOOTPRINTING_",pheno_data[i,"SID"],"_",project_name,".png", sep = "")
    ...
    write.csv(results[['p13data']][[i]], paste(cdir, "13SCA_TABLE_scATAC_TOP_ENRICHED_MOTIFS_",pheno_data[i,"SID"],".csv",sep = ""), row.names = F, quote = F)
    ...
    somePNGPath <- paste(cdir,"14SCA_PLOT_scATAC_TOP_ENRICHED_MOTIFS_",pheno_data[i,"SID"],"_",project_name,".png", sep = "")
    ...
    somePNGPath <- paste(cdir,"15SCA_PLOT_scATAC_UMAP_VS_TOP_ENRICHED_MOTIFS_",pheno_data[i,"SID"],"_",project_name,".png", sep = "")
  ...
  if((length(results[['data']]) > 1) & (length(data_ref) == 1)){
    ...
    write.csv(results[['data_markers']], paste(cdir, "16SCA_TABLE_scATAC_TOP_PEAKS_MERGED_SAMPLES.csv",sep = ""), row.names = F, quote = F)
    ...
    somePNGPath <- paste(cdir,"16SCA_PLOT_scATAC_UMAP_INTEGRATED_ATAC_SAMPLES_AUTOCLUST_",project_name,".png", sep = "")
    ...
    somePNGPath <- paste(cdir,"17SCA_PLOT_scATAC_INTEGRATED_ATAC_SAMPLES_UMAP_DENSITY_TOP_1_MARKER_IN_CLUSTERS_",project_name,".png", sep = "")
    ...
    for(k in 1:length(current_clusters)){
      if(length(which(unlist(top_n$cluster) == current_clusters[k])) > 0){
        ...
        somePNGPath <- paste(cdir,"18SCA_PLOT_scATAC_INTEGRATED_ATAC_SAMPLES_TOP_",n,"_MARKERS_IN_CLUSTER_",current_clusters[k],"_",project_name,".png", sep = "")
    ...
    write.csv(results[['data_activity_markers']], paste(cdir, "18SCA_TABLE_scATAC_TOP_GENES_MERGED_SAMPLES.csv",sep = ""), row.names = F, quote = F)
    ...
    for(k in 1:length(clusters)){
      if(length(which(unlist(top1$cluster) == clusters[k])) > 0){
        ...
        somePNGPath <- paste(cdir,"19SCA_PLOT_scATAC_INTEGRATED_SAMPLES_Tn5_INSERTION_COVERAGE_TOP_PEAK_GENE_ACTIVITY_IN_CLUSTER_",current_clusters[k],"_",project_name,".png", sep = "")
  ...
  saveRDS(results, paste(cdir,"20SCA_DATA_scATAC_",project_name,".RDS", sep = ""))
